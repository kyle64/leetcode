...
public class Q8StringtoInteger {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (current >= 0 && current <= 9) {
                ...
                if (signal > 0
                        ...
                        || (result == Integer.MAX_VALUE / 10 && current > Integer.MAX_VALUE % 10))) {
                    return Integer.MAX_VALUE;
                } else {
                    // 负数
                    if (result < Integer.MIN_VALUE / 10
                            || (result == Integer.MIN_VALUE / 10 && current * -1 < Integer.MIN_VALUE % 10)) {
                        return Integer.MIN_VALUE;
                    }
                }

                // 累加
                result = result * 10 + signal * current;
            } else {
                break;
            }
        }
    ...
    public static void main(String[] args) {
        Q8StringtoInteger q8StringtoInteger = new Q8StringtoInteger();
        String input = "91283472332";
        System.out.println(q8StringtoInteger.myAtoi(input));
    }
}
